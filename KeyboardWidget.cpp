...
KeyboardWidget::KeyboardWidget(QWidget *parent)
    : QWidget(parent) {
    setStyleSheet("QPushButton { font-size: 14pt; min-width: 45px; min-height: 45px; }");
    setupUI();
    setupKeyLayouts();
    updateKeyboardUI();
}
...
// Reszta setupKeyLayouts jest identyczna, ale z dodaniem mapowania klawiszy PL
void KeyboardWidget::setupKeyLayouts(){
    ...
    mainLayout->addWidget(createSpecialKey("DEL", 67), 0, 12, 1, 2);
    ...
    // Row 2
    mainLayout->addWidget(createSpecialKey("TAB", 61), 2, 0, 1, 1);
    ...
    mainLayout->addWidget(createSpecialKey("ENTER", 66), 2, 12, 1, 2);
    
    // Row 3
    shiftLeftButton = new QPushButton("SHIFT");
    ...
    shiftRightButton = new QPushButton("SHIFT");
    ...
    // Row 4
    mainLayout->addWidget(createSpecialKey("CTRL", 113), 4, 0, 1, 2);
    mainLayout->addWidget(createSpecialKey("ALT", 57), 4, 2, 1, 2);
    capsLockButton = new QPushButton("CAPS");
    ...
    mainLayout->addWidget(createSpecialKey("SPACE", 62), 4, 6, 1, 6);
    mainLayout->addWidget(createSpecialKey("ALT", 57), 4, 12, 1, 1); // Zmieniono kolumnę na 12 i rozpiętość na 1
    mainLayout->addWidget(createSpecialKey("CTRL", 113), 4, 13, 1, 1); // Zmieniono kolumnę na 13 i rozpiętość na 1
    
    // Row 5 (Navigation and media keys)
    mainLayout->addWidget(createSpecialKey("HOME", 3), 5, 0);
    mainLayout->addWidget(createSpecialKey("BACK", 4), 5, 1);
    mainLayout->addWidget(createSpecialKey("POWER", 26), 5, 2);
    mainLayout->addWidget(createSpecialKey("MENU", 82), 5, 3);
    mainLayout->addWidget(createSpecialKey("VOL UP", 24), 5, 4);
    mainLayout->addWidget(createSpecialKey("VOL DOWN", 25), 5, 5);
    mainLayout->addWidget(createSpecialKey("UP", 19), 5, 7);
    
    // Row 6
    mainLayout->addWidget(createSpecialKey("LEFT", 21), 6, 6);
    mainLayout->addWidget(createSpecialKey("RIGHT", 22), 6, 8);
    
    // Row 7
    mainLayout->addWidget(createSpecialKey("DOWN", 20), 7, 7);
    ...
    // Dodanie klawiszy dla polskiego układu, aby ułatwić przełączanie
    // Zmieniamy klawisze, które będą wyświetlać polskie znaki w trybie PL
    // a -> ą, c -> ć, e -> ę, l -> ł, n -> ń, o -> ó, s -> ś, z -> ź, x -> ż
...
void KeyboardWidget::onKeyClicked(){
    ...
    QString command;
    if (isPolishLayout) {
        // W trybie PL litery są wysyłane jako "input text"
        QString charToSend;
        if (isShiftActive || isCapsActive) {
             // W trybie PL, duże litery to litery PL (Ą, Ć, itp.)
             ...
             else charToSend = button->property("textShift").toString(); // Zwykłe symbole, np. !, @, itp.
        } else {
             // Małe litery PL (ą, ć, itp.)
             if (button->property("textNormal").toString() == "a") charToSend = "ą";
             else if (button->property("textNormal").toString() == "c") charToSend = "ć";
             else if (button->property("textNormal").toString() == "e") charToSend = "ę";
             else if (button->property("textNormal").toString() == "l") charToSend = "ł";
             ...
             else charToSend = button->property("textNormal").toString(); // Zwykłe symbole
        }
        
        // Ważne: W trybie PL wysyłamy "input text" dla klawiszy PL i normalnych liter/cyfr
        ...
        if (isShiftActive || isCapsActive) {
            // W trybie SHIFT/CAPS wysyłamy kod klawisza + meta 2 (SHIFT)
    ...
    // Jeśli użyto jednorazowego SHIFT, wyłącz go
    if (isShiftActive && !isCapsActive) {
        isShiftActive = false;
...
void KeyboardWidget::onSpecialKeyClicked(){
    ...
    if (!button) return;
    int keyCode = button->property("androidKeyCode").toInt();
    // Komenda zawsze jest keyevent (nawet w trybie PL)
    emit adbCommandGenerated(QString("input keyevent %1").arg(keyCode));
}

void KeyboardWidget::onPolishKeyClicked(){
    // Ta metoda jest zbędna, ponieważ polskie znaki są obsługiwane przez onKeyClicked w trybie isPolishLayout
}

void KeyboardWidget::onShiftToggled(bool checked){
    isShiftActive = checked;
    // Synchronizacja obu przycisków
...
void KeyboardWidget::updateKeyboardUI(){
    ...
    for (auto& pair : keyMap.toStdMap()) {
        ...
        // Zwykłe klawisze (cyfry i symbole)
    ...
    if (isPolishLayout) {
        ...
        // Przywrócenie do QWERTY (tylko małe lub duże)
