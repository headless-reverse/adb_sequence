...
SwipeBuilderWidget::SwipeBuilderWidget(CommandExecutor *executor, QWidget *parent)
    ...
    tabWidget->addTab(m_canvas, "Screen Capture (Swipe/Tap)");
    m_keyboardWidget = new KeyboardWidget(this);
    tabWidget->addTab(m_keyboardWidget, "Virtual Keyboard (Text/Keys)");
    ...
    connect(m_adbProcess, &QProcess::errorOccurred, this, [this](QProcess::ProcessError error){
        ...
        if (error == QProcess::FailedToStart) {
            emit adbStatus("ADB command not found! Check if path is set correctly or if 'adb' is in PATH.", true);
        } else {
    ...
    if (exitCode == 0) {
        ...
        if (!data.isEmpty()) {
            ...
            emit adbStatus("Monitoring: OK", false);
             ...
             emit adbStatus("ADB returned success but sent no image data. Is the device connected and authorized?", true);}
    } else {
        QByteArray errData = m_adbProcess->readAllStandardError();
        QString errorMsg = QString("ADB failed (Code: %1). Check device connection/root/authorization. Error Output: %2")
                           .arg(exitCode)
...
void SwipeBuilderWidget::runSelectedAction() {
    ...
    if (a.type == SwipeAction::Tap) {
        ...
        if (shellArgs.isEmpty()) {
            emit adbStatus("CMD: Command is empty after parsing.", true);
            return;}
...
void SwipeBuilderWidget::onAdbCommandFinished(int exitCode, QProcess::ExitStatus exitStatus) {
    ...
    if (exitCode == 0) {
        ...
        emit adbStatus(QString("Live execution failed with code: %1. Check the Log tab for details.").arg(exitCode), true);}
    startMonitoring();}
