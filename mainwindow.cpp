...
QWidget* MainWindow::createSequenceTabWidget() {
    ...
    tabWidget->addTab(sequenceRunnerWidget, "Sequence Runner");
    return tabWidget;}
...
QWidget* MainWindow::createControlsWidget() {
    ...
    timeLayout->addWidget(m_intervalToggle);    
    m_commandTimerLabel = new QLabel("(0 s)");
    m_commandTimerLabel->setStyleSheet("color: black; font-weight: normal;");
    timeLayout->addWidget(m_commandTimerLabel);
...
/*
void MainWindow::onSequenceFileGenerated(const QString &filePath) {
    m_sequenceRunner->clearSequence();
    if (m_sequenceRunner->appendSequence(filePath)) {
        appendLog(QString("Sequence generated and loaded: %1").arg(QFileInfo(filePath).fileName()), "#4CAF50");
        if (m_dockSequence) {
            m_dockSequence->setVisible(true);
            m_dockSequence->raise();}
    } else {
        appendLog(QString("Failed to load generated sequence: %1").arg(QFileInfo(filePath).fileName()), "#F44336");}}
*/
